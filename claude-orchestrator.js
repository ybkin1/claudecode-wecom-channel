    ...
    try {
      ...
      if (fileContext) {
        ...
        if (Buffer.byteLength(fileContext, 'utf8') > MAX_FILE_CONTEXT_LENGTH) {
          // H-2 fix: 按字节安全截断，回退到 UTF-8 完整字符边界
          var fcBuf = Buffer.from(fileContext, "utf8");
          var fcTrunc = fcBuf.slice(0, MAX_FILE_CONTEXT_LENGTH);
          var lb = fcTrunc[fcTrunc.length - 1];
          while (fcTrunc.length > 0 && (lb & 0xC0) === 0x80) {
            fcTrunc = fcTrunc.slice(0, -1);
            lb = fcTrunc[fcTrunc.length - 1];
          }
          safeFileContext = fcTrunc.toString("utf8").replace(/[\x00\uFFFD]/g, "")
            + '\n\n... (文件内容过长，已截断)';
        }
